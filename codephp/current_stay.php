...
<?php
if(isset($_POST['checkin']) || isset($_POST['checkupdate']) || isset($_POST['checkout'])){
    ...
    if(array_filter($errors)){  // any error in every field
            // echo 'erros in the form';
    } else {        
        if ($bookingid == ""){  // GUEST DOESN'T RESEVE BEFORE
            // ADD GUEST TABLE
            $query = "INSERT INTO `Guest` (`Guest_ID`, `Photo`, `Lname`, `Fname`, `Phone_Number`, `Address`, `Email`, `ID_Info`, `Vehicle`, `License_Plate`) VALUES (NULL, NULL, '".$fname."', '".$lname."', NULL, NULL, NULL, NULL, NULL, NULL);";
            DB::query($query);


            // ADD BOOKING TABLE (no website resevation) ( to get $bookingid )      
            $today = date('Y-m-d');            
            $query = "INSERT INTO `Booking` (`BookingID`, `Guest_ID`, `Date_Made`, `Website_Reservation`) VALUES (NULL,(SELECT MAX(Guest_ID) FROM Guest),'".$today."', '0');";
            DB::query($query);            

            // must have $guestid & $bookingid before continue
            $query = "SELECT MAX(Guest_ID) AS Guest_ID FROM Guest";
            $guestid_list = DB::query($query);
            $guestid = $guestid_list[0]['Guest_ID'];

            $query = "SELECT MAX(BookingID) AS BookingID FROM Booking";
            $bookingid_list = DB::query($query);
            $bookingid = $bookingid_list[0]['BookingID'];

            $record_status_code = 0; // force running the one of the branch of if-statments below// ALL CHECK-IN STEPS
            
        }
            
        // CHECK-IN, UPDATE, CHECK OUT STEPS
        if ($record_status_code == 0 || (isset($_POST['checkupdate'])) ) { // IF CHECK IN, UPDATE RECORD TABLE, ADD INVOICE TABLE
            ...
            if (!empty($bookinglist)){
                ...
                if (!isset($_POST['checkupdate']))   {
                    ...
                    $bookinglist = DB::query($query);
                    // ADD INVOICE ROOM TABLE
                    $query = "INSERT INTO Invoice_Rooms (Invoice_ID, Room_Number) VALUES ((SELECT Invoice_ID From Invoice ORDER BY Invoice_ID DESC LIMIT 1),('".$room_number."'));";
                    DB::query($query);
                    // UPDATE ROOM STATUS to Occupied
                    $query = "UPDATE `Room` SET `Room_Status_ID` = '1' WHERE `Room`.`Room_Number` = ".$room_number;
                    DB::query($query);
                    // HOUSEKEEPING TABLE 
                    $query = "UPDATE `Housekeeping` SET `Bathroom` = '0', `Towels` = '0', `Bed_Sheets` = '0', `Vacuum` = '0', `Dusting` = '0', `Electronics` = '0' WHERE `Housekeeping`.`Room_Number` = ".$room_number;
                    DB::query($query);
                    header("Location: ./index.php");
                } else { // IF CHECK UPDATE, UPDATE RECORD TABLE (above if), UPDATE INVOICE TABLE

                    $query = "UPDATE `Invoice` SET `Date_Checkout` = '".$created_date2."', `Payment_Made` = '".$paid."', `Total_Charge` = '".$total_charge."' WHERE `Invoice`.`Booking_ID` = ".$bookingid;
                    $bookinglist = DB::query($query);                                           
                }
                header("Location: ./cap6-current_stay.php?bookingid=".$bookingid);
            }

            } elseif ($record_status_code == 1) {  // IF CHECK OUT, 
                // UPDATE RECORD TABLE, 
                $query = "UPDATE Record SET Record_Status_Code = 2 WHERE BookingID = ".$bookingid;
                $bookinglist = DB::query($query);

                // UPDATE INVOICE TALBE// abc3
                date_default_timezone_set('Etc/GMT'); //Make sure the time is GMT
                $today = date('Y-m-d H:i:s');
                $created_date2 = date("Y-m-d H:i:s",strtotime( $today." GMT+8")); 
                // $created_date2 = date("Y-m-d H:i:s");
                $query = "UPDATE `Invoice` SET `Date_Checkout` = '".$created_date2."', `Payment_Made` = '".$paid."',`Completed` = '1', `Total_Charge` = '".$total_charge."' WHERE `Invoice`.`Booking_ID` = ".$bookingid;
                $bookinglist = DB::query($query);  
                // Update Room Status to dirty
                $query = "UPDATE `Room` SET `Room_Status_ID` = '3' WHERE `Room`.`Room_Number` = ".$room_number;
                DB::query($query);
                header("Location: ./index.php");

            } elseif ($record_status_code == 2) {
                // should disable all of input, but no link links to it, so don't need to implement.
...
<?php if ($bookingid != ""): ?>                
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    <h4 class="text-light"><a href="./edit_guest_info.php?guestid=<?php echo htmlspecialchars($guestid); ?>" class="m-t-5"><?php echo htmlspecialchars($fname).' '.htmlspecialchars($lname); ?></a></h4>
                </div>
            </div>
